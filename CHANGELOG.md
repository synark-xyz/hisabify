...
### Added
- **Manage Subscription page**: `/settings/subscription` is now the single manage surface, reachable from `Settings → Subscription` and from `Profile → Personal` (which navigates here instead of mounting its own trigger). It shows a live status card — plan term, renews-on / access-until, store, trial and billing-issue notices — and delegates every action: the native Customer Center for cancel/change-plan/refund, `UpgradeModal` for upgrades, `restorePurchases` for restores. The branching is a pure function, `deriveSubscriptionStatus()` in `src/lib/subscriptionStatus.ts`, returning `pro | referral | free | unavailable`. It reads RevenueCat directly, never the `users.subscription_type` mirror. On web — where RevenueCat is not available at all — it says subscriptions are managed in the app rather than telling a paying subscriber they are on the free plan.
- **Unified error and offline UI**: `ErrorState` / `DataErrorState` (`src/components/ErrorState.tsx`) replace the silent empty states that a failed fetch used to render — a dead connection previously showed "No debts tracked", "All caught up" and empty charts, indistinguishable from a clean account. Four variants (`offline | server | notFound | generic`) classified by the pure `toErrorVariant()` (`src/lib/errorState.ts`) over a `toAppError()` taxonomy that now reads `status` off plain PostgREST objects and matches all three engines' spellings of a transport failure. `useOnline()` drives a top-anchored `OfflineBanner` from `Layout`; a `NETWORK_ERROR` still classifies as offline even when the device claims to be online (captive portals). `ErrorBoundary` is now mounted per route as well as at the app root, keyed on the pathname, and retries by remounting the subtree. `useAuth`'s session bootstrap is bounded by a timeout and clears `loading` on every path, so a hung `getSession()` can no longer spin `ProtectedRoute` forever. Copy added to all three locales under the existing `errors.*` block.
- **Pro gates on savings goals and report export**: both are now enforced at the call site (`SavingsTabContent`, `ReportsPage`), not only advertised. `UpgradeModal`'s claims were corrected to match what is actually gated.
- **Ad verification and migration tooling**: `npm run verify:apk-ads` reads a built APK's real package name, manifest AdMob app ID and its own minified bundle to assert the per-variant ad policy (staging entirely on Google's test publisher, release on the exact `ADMOB_APP_ID_RELEASE`), and warns when the APK predates `ads.ts`, `useAdBanner.ts`, `useRevenueCat.ts` or `build.gradle`. `npm run verify:migration` runs the stale-pro-mirror migration against a throwaway Postgres carrying the real CHECK constraint; `npm run migration:apply` (and `migration:apply:test`) is the narrow applier for it.
- **Ads (Android, free tier)**: An anchored AdMob banner is shown to signed-in, non-premium users on Android. Pro removes it. The banner appears on Layout-group routes only — never on Settings, Profile, or the legal pages. Google's UMP consent form is shown before the first ad request where required, and can be reopened from `Profile → Data & Privacy → Ad privacy settings`. Requires `VITE_ADMOB_BANNER_ID`; falls back to Google's test ad unit in any non-production build. The native AdMob application ID follows the same rule: debug and staging builds always use Google's test app ID, and release builds use `ADMOB_APP_ID_RELEASE` from `android/local.properties` (or the environment), warning at build time if it is unset.
...
### Changed
- **Transaction row actions — long press replaces swipe**: `TransactionItem` no longer drags horizontally. Press and hold (500ms, cancelled by 10px of travel) reveals Edit and Delete. Swipe-to-reveal fought the page's own scrolling and fired on scroll momentum; a movement-cancelled long press cannot. Applies everywhere the component renders — Dashboard activity card, Expenses, `/activity`. The hold only arms when `onEdit`/`onDelete` are passed, so `BudgetTransactionsSheet` no longer reveals dead buttons. Keyboard and screen-reader users reach the same actions via the transaction details page, which is unchanged.
- **Activity history is one feed, defined once**: `mergeActivityFeed()` in `src/lib/activityFeed.ts` is now the single definition of the merged `activity_log` + `transactions` feed; Dashboard renders its first 5 and `/activity` renders all of it. The description parser (the `key|param` format plus the legacy English fallback) moved there too as `formatActivityDescription()`. Both previously lived inline in `Dashboard.tsx`, which is how the two screens drifted apart.
- **Page transitions removed**: Route-level transitions are gone. Animating the whole route tree fought with the scroll restore in `Layout` and the Suspense fallback swap, which read as a glitch on every navigation. `src/lib/pageMotion.ts` now exports no-op variants and `PageTransition.tsx` is a plain container; per-component motion inside a page is unaffected. (The shared fade-and-lift definition added earlier while unreleased is what this replaces.)
- **Banner ad lifecycle moved into a component**: the native banner is now owned by `BannerAd` (`src/components/BannerAd.tsx`), mounted once in `Layout` — that mount point *is* the placement policy. `src/hooks/useAdBanner.ts` keeps only what is shared: `initAdMob()` (the memoised `initialize → requestConsentInfo → showConsentForm` order), `getNativeAppId()` and `useAdPrivacyOptions()`. The banner publishes `--ad-banner-h` instead of hardcoding `+50px` into `.pb-page-content` (which reserved dead space for Pro users), anchors `BOTTOM_CENTER` rather than over the header, and reuses the memoised consent init instead of skipping it. Height bookkeeping is the pure, event-order-independent `createBannerHeightTracker()` in `src/lib/ads.ts`.
- **Bottom inset has exactly one owner**: `Layout`'s `<main>` now carries both `min-h-screen` and `.pb-page-content`; pages and `PageShell` (when `withBottomNav`) no longer declare either.
- **Sign out is hardened**: `signOut()` now checks the `{ error }` Supabase returns, falls back to `scope: 'local'` so signing out never depends on the network, and verifies the session is actually gone before navigating.
- **Free-plan subscription actions**: Cancel is shown only for an actual paying subscriber — free and referral users have nothing to cancel — so the fallback to the store's subscription page is gone.
...
### Fixed
- **Sign out failed silently and left the user signed in**: `supabase.auth.signOut()` *returns* `{ error }` rather than throwing, so the `try/catch` around it caught nothing and every failure looked like success. Worse, gotrue bails out before `_removeSession()` when the server call fails, so the local session survived — the app toasted "Signed out successfully", navigated to `/auth`, and the user was still authenticated. Now the returned error is checked, a `scope: 'local'` sign-out is attempted so the operation never depends on the network, the session is verified gone, and a real failure surfaces instead of navigating away. Regression cover in `src/hooks/__tests__/useAuth.signout.test.tsx`.
- **Bottom navigation disappeared**: the nav is `position: fixed` but had lost its bottom anchor, so it sat at its static position in flow — off-screen. The existing suite passed throughout, because every tab still *renders* when the bar is invisible; the new test pins the anchor itself.
- **Ad banner covered the bottom navigation**: re-anchoring the nav with a hardcoded `bottom-0` put it exactly where the native banner sits. The `var(--ad-banner-h, 0px)` offset is restored — the variable is `0px` when no ad shows, so it is `bottom-0` anyway in that case. Separately, the height tracker gated `SizeChanged` behind a `loaded` flag, but on device `SizeChanged` fires *before* `Loaded`, so the only size event was discarded and the reserved height stayed `0`. Both event orderings are now covered by tests.
- **Over-scroll past the end of content**: the bottom inset was declared on several nested boxes at once. On the Dashboard it stacked three deep (`.pb-page-content` on the Layout `<main>`, again on the `PullToRefresh` wrapper, plus `pb-24` on the inner `<main>`) — 376px of empty scrollable space below the last element. Pages also set `min-h-screen` inside a `min-height`-less Layout main that carried the padding, so the inset extended the viewport rather than being absorbed by it. With a single owner and `border-box`, dead space at full scroll on a 390×844 viewport goes from 376px to 140px — exactly the nav + FAB reserve. Also dropped `fade-bottom-overlay` from Budget and Expenses: the mask was applied to the full-height scrolling box rather than a viewport-fixed overlay, so it faded out content in the middle of the document.
- **Staging builds paired a test AdMob app ID with the real ad unit**: `npm run build && npx cap sync` emits a production web bundle regardless of which APK variant wraps it, so `import.meta.env.PROD` alone was not enough. `getBannerUnitId()` now also requires the native package to be `io.synark.hisabify` (not `.staging`). AdMob will not serve that mismatched combination, and `useAdBanner` swallows ad failures by design, so ads simply disappeared with no visible error.
- **Subscription downgrades were never mirrored**: `users.subscription_type` was written only on upgrade, so expiries, refunds and cancellations — which arrive solely through `addCustomerInfoUpdateListener` and the foreground refresh — left rows stuck at `pro`/`active` forever. The write moved into `updatePremiumState`, which sees every transition in both directions. The downgrade value is `'base'`, not `'free'` (the DB CHECK is `('base','pro')`). Migration `20260828000000_reset_stale_pro_mirror.sql` repairs the rows the old code stranded, matching on `lower(btrim(email))` — an exact `IN (...)` silently no-ops on a differently cased address, and a migration that updates zero rows looks exactly like one that worked.
- **Two-way mirror wrote on every free-user cold start**: the last mirrored value is now cached per user in `localStorage`, and only *after* the update is confirmed to have matched a row — so a failed or zero-row write retries next launch rather than being cached as done.
- **Upgrade banner was unreadable in light mode**: the Dashboard Pro banner used hardcoded white-on-gradient. It now uses `bg-primary` / `text-primary-foreground` theme tokens, so it follows the active theme.
- **Analytics breakdown card looked identical collapsed and expanded**: the two-column grid stretched both cards to the tallest row height. Added `items-start`.
- **Dashboard content was clipped behind the bottom nav and ad banner**: the page hardcoded `pb-24` (96px) for its bottom padding while the nav, FAB and banner need `140px + var(--ad-banner-h) + env(safe-area-inset-bottom)` — the clearance `.pb-page-content` exists to provide, and which `ExpensesPage`/`BudgetPage` already used. With a banner showing (every free Android user) the last rows of the activity feed sat underneath the nav and could not be reached. Both the main scroll container and the loading skeleton now use `.pb-page-content`.
- **`/activity` was empty while the Dashboard showed data**: `activity_log` only ever receives debt events — the only writer is `useAnalytics.logActivity`, reached solely from debt CRUD (11 users have transactions; 3 have any activity row, all debts). The Dashboard looked populated because it merged in `transactions`; the page rendered `activity_log` alone. Both now render the same merged feed. `useActivityHistory.logActivity` still has zero callers — wiring the remaining writers (transaction, budget, savings, card) into `activity_log` is the real long-term fix, and the page's icon map already anticipates those types.
- **RevenueCat premium mirror wrote zero rows, silently**: `syncPremiumToSupabase()` and both webhook update paths matched on `users.id` while passing the Supabase auth uid — but `id` is a `gen_random_uuid()` surrogate PK and `user_id` is the auth column. Zero rows matched, PostgREST returned 204, and `error` was `null`, so nothing ever logged. Now matches on `user_id`, selects back, and warns on an empty match.
- **RevenueCat entitlements leaked between accounts on one device**: the configure guard was a bare boolean that never reset, and `signOut()` never called `Purchases.logOut()`. After user A signed out, user B signing in on the same process hit the early return, so `logIn` never ran for B and B saw A's entitlement. The guard now keys on the user id and sign-out clears the RevenueCat identity.
- **RevenueCat webhook wrote a value the schema rejects**: the deactivate path set `subscription_type: 'free'` against `CHECK (subscription_type IN ('base','pro'))`. Masked by the `id`/`user_id` bug above — fixing that alone would have surfaced a 500 and an endless RevenueCat retry loop, so both ship together.
- **Subscription expiry mid-session went unnoticed, and a listener leaked**: there was no `addCustomerInfoUpdateListener`, so a subscription ending while the app was open left Pro unlocked until the next background→foreground cycle. Separately, the `appStateChange` cleanup was returned from inside an async IIFE and discarded, adding a native listener on every remount. Both fixed in one effect.
- **Receipt scanning failed with an unactionable error**: the guard was `if (!apiKey)`, but the placeholder `your_gemini_api_key` is truthy, so every scan round-tripped a bad key to Google and surfaced as "Scan Failed — try again" for something retrying could never fix. The guard now rejects placeholder-shaped keys and reports "Receipt Scanning Unavailable" instead. (`gemini-2.5-flash` on `v1beta` was verified current — not a factor.)
- **Receipts wrote multi-megabyte data URLs to the database**: `receiptUrl` carried the raw camera file as base64 straight into `transactions.receipt_url` — two production rows hold one, the largest 2.5 MB, re-fetched on every `select *` of the transaction list. Now reuses the already-compressed ~150 KB image. Existing rows are untouched: `UPDATE public.transactions SET receipt_url = NULL WHERE receipt_url LIKE 'data:%';` clears them.
- **Theme selection**: Fixed the app rendering Light on a dark device. Picking "System" no longer freezes into whatever it resolved to at that moment — `system` is stored as-is and re-resolved live against `prefers-color-scheme`. The light/dark class is now applied from `localStorage` before first paint (inline script in `index.html`) instead of waiting on a Supabase round-trip that could never complete offline or logged out, leaving the DOM on the light `:root` default. Settings no longer keeps a second copy of the theme; `useTheme` is the only source of truth. Dashboard and the particle background branched on a non-existent `variant` field / the raw `theme` instead of `resolvedTheme`.
