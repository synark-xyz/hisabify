        ...
        <div className="bg-card/95 backdrop-blur-sm border border-border rounded-lg px-4 py-3 shadow-lg">
          ...
          <div className="space-y-1">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">{t('budget.budget')}:</span>
            ...
            <div className={`text-xs font-medium ${isOver ? 'text-destructive' : 'text-emerald-500'}`}>
              {isOver
                ? t('budget.overBudget', { amount: formatAmount(Math.abs(diff)) })
                : t('analytics.underBudget', { amount: formatAmount(Math.abs(diff)) })}
            </div>
          </div>
        </div>
      );
            ...
            <Legend
              ...
              formatter={(value) => (
                <span className="text-foreground capitalize">
                  {value === 'budget' ? t('budget.budget') : t('analytics.actual')}
                </span>
              )}
            />
            <Bar
